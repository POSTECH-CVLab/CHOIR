...
from canonical_capsules.models.model_utils.decode_utils import ChamferLoss, KpDecoder, MlpPointsFC
from canonical_capsules.models.model_utils.acne_utils import AcneKpEncoder
from canonical_capsules.geom_torch import trans_pc_random, procruste_pose, get_rots 
from canonical_capsules.loss_util import *
from canonical_capsules.vis_util import *
...
class AcneAeAligner(AcneAe):
    def forward_align(self, pc, rt=None, mode="test", iter_idx=None, writer=None):
        ...
        x_can = torch.matmul(R_can, x) + T_can # R_can.shape = (B, 3, 3) x.shape = (B, 3, 1024)
        ...
        if mode == "train":
            ...
            ret_dict["loss"] = loss
            ret_dict["x_canonical"] = x_can[:bs].transpose(2, 1)
        else:
            ret_dict["att_aligner"] = att
            ret_dict["kps_ref"] = kps_ref
            ret_dict["kps"] = kps
            ret_dict["rt_can"] = [R_can, T_can] 
            ret_dict["x_canonical"] = x_can.transpose(2, 1)
            ret_dict["end_rots"] = R_can.transpose(1, 2)
