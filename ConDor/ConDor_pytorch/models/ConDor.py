import torch
from .TFN import TFN
from .layers import MLP, MLP_layer
...
from .layers import MLP, MLP_layer, set_sphere_weights, apply_layers, type_1
from ..utils.pooling import kd_pooling_1d
from ..utils.train_utils import orthonormalize_basis
from ..utils.pointcloud_utils import convert_yzx_to_xyz_basis
...
class ConDor(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        _, indices = torch.topk(-error_full, k = 1)
        ...
        canonical_x = torch.einsum('bij,bkj->bki', basis_instance_to_canonical, x) # canonical_x = x @ basis_instance_to_canonical.transpose(1, 2)
        out["x_canonical"] = canonical_x
        out["end_rots"] = basis_instance_to_canonical.transpose(1, 2)
